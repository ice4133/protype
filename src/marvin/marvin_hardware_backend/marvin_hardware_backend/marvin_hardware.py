...
class MarvinHardwareSession:
    ...
    def connect_and_prepare(
        ...
        *,
        ...
        include_position_reference: bool = False,
    ) -> MarvinFeedback | tuple[MarvinFeedback, MarvinFeedback]:
    ...
    @staticmethod
    def _state_can_still_reach_target(
        *, state: int, error: int, target_state: int
    ) -> bool:
        if error == 0 and state in (0, 1, 2, 3, 101, 102, 103):
            return True
        # Controller system version 100343009 was observed reporting 109 on
        # both arms immediately after a state-0 request, with cmd_state=-1
        # and err_code=0. Treat it only as a bounded idle-transition state;
        # _wait_for_state still requires state 0 before its timeout.
        if target_state == 0 and state == 109 and error == 0:
            return True
    ...
    def shutdown_verified(
        ...
        *,
        ...
        """Enter state 0, verify feedback, then release the SDK session."""
        if not self._connected:
            raise MarvinHardwareError(
                "cannot verify shutdown for a disconnected session"
            )
        try:
            self._begin_command_batch()
            for arm in ("A", "B"):
    ...
    def _release_after_prepare_failure(self, *, soft_stop: bool = True) -> None:
        if not self._connected:
            return
        if soft_stop:
            try:
